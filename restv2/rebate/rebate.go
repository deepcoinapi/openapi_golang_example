...
func (t *RebateCtrl) AgentsRebateList() {
	requestURL := fmt.Sprintf(t.env.Url+consts.AGENTS_REBATE_LIST+"?uid=%d&type=%d&startTime=%d&endTime=%d&pageNum=%d&pageSize=%d", 36007196, 0, 1700000000, 1800000000, 1, 100)
	requestPath := fmt.Sprintf(consts.AGENTS_REBATE_LIST+"?uid=%d&type=%d&startTime=%d&endTime=%d&pageNum=%d&pageSize=%d", 36007196, 0, 1700000000, 1800000000, 1, 100)
	signature.DoHttp(requestURL, consts.HTTP_METHOD_GET, requestPath, "", &t.env)
}

// AgentsRebates 返佣汇总查询
func (t *RebateCtrl) AgentsRebates() {
	requestURL := fmt.Sprintf(t.env.Url+consts.AGENTS_REBATES+"?uid=%d&type=%d&startTime=%d&endTime=%d", 36007196, 0, 1700000000, 1800000000)
	requestPath := fmt.Sprintf(consts.AGENTS_REBATES+"?uid=%d&type=%d&startTime=%d&endTime=%d", 36007196, 0, 1700000000, 1800000000)
	signature.DoHttp(requestURL, consts.HTTP_METHOD_GET, requestPath, "", &t.env)
}
